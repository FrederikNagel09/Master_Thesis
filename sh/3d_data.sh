#!/bin/bash
#BSUB -J weight-probability-3D-data      # Job name
...
#BSUB -o src/outputs/weight-probability-3D-data.out                        # Standard output redirection
#BSUB -e src/outputs/weight-probability-3D-data.err                        # Standard error redirection
...
python /zhome/66/4/156534/Master_Thesis/main.py\
    --run_name weight-probability-3D-data \
    --epochs 700 \
    --batch_size 64 \
    --subset_frac 1.0 \
    --n_fid_samples 1024 \
