...
python src/scripts/analyze_latent_diffusion.py \
    --out_dir analysis_results/Latent_Diffusion_working \
    --config_path src/train_results/latent-diffusion/metadata/config.json \
    --weights_path src/train_results/latent-diffusion/weights/weights.pt
...
sys.path.append(".")
# Import the requested latent diffusion model builder
...
@torch.no_grad()
def get_generated_latent_outputs(
    model: LatentDiffusion,
    n_samples: int,
) -> dict[str, torch.Tensor]:
    """
    Generate latent samples and decode them into images.

    Returns:
        latent_samples : (B, latent_dim, H', W')
        reconstructions: (B, data_dim)
    """
    model.eval()

    latent_samples = model._sample_latent(
        n_samples=n_samples,
        collect_snapshots=False,
        debug=False,
    )

    reconstructions = model._decode_latent(latent_samples)
...
def get_encoder_outputs(
    ...
    """
    Collect encoder outputs and reconstructions.

    Returns:
        z               : sampled latent
        mu              : encoder mean
        logvar          : encoder log variance
        reconstructions : decoded images
    """
    model.eval()
    ...
    for i, (x, _) in enumerate(loader):
        ...
        if x.dim() == 2:
            channels = model.data_dim // (model.img_size * model.img_size)
            x = x.view(B, channels, model.img_size, model.img_size)

        z, mu, logvar = model.encode(x)
        pred, theta_decoded = model.decoder.forward_with_weights(z)

        bufs["z"].append(z.cpu().numpy())
        bufs["mu"].append(mu.cpu().numpy())
        bufs["logvar"].append(logvar.cpu().numpy())
        bufs["theta_decoded"].append(theta_decoded.cpu().numpy())

    return {k: np.concatenate(v, axis=0) for k, v in bufs.items()}


def load_model(cfg_path: str, ckpt_path: str, device: torch.device) -> torch.nn.Module:
    """
...
def plot_histograms(
    ...
    for label, arr in arrays.items():
        ax.hist(arr.flatten(), bins=n_bins, alpha=0.55, label=label, density=True)
    ax.set_title(title)
    ax.set_xlabel("value")
...
def _get_parser():
    ...
    parser.add_argument(
        "--n_samples", type=int, default=4, help="samples for generated weight analysis"
    )
...
def posterior_collapse_check_multiple(
    ...
    for lbl in sorted(class_images.keys()):
        ...
        logvar_expanded = logvar.expand(n_samples, -1, -1, -1)  # (N, mod_dim)
        theta_raw = model.latent_encoder.reparameterize(
            mu_expanded, logvar_expanded
        )  # (N, mod_dim)

        pred, theta_decoded = model.decoder.forward_with_weights(theta_raw)

        all_theta_raw.append(theta_raw.cpu().numpy())
        all_theta_decode.append(theta_decoded.cpu().numpy())
        all_mu.append(mu.cpu().numpy())
        all_logvar.append(logvar.cpu().numpy())
        labels.append(lbl)

    all_theta_raw = np.stack(all_theta_raw, axis=0)  # (10, N, mod_dim)
    all_theta_decode = np.stack(all_theta_decode, axis=0)  # (10, N, mod_dim)
...
def plot_pca_posterior_multiple(
    ...
    datasets_to_plot = [
        ...
        ("theta_decode", collapse_outputs["mu"], "PCA — θ decoded samples"),
    ]
    ...
    for ax, (key, mu_all, title) in zip(axes, datasets_to_plot):
        ...
        flat = data_all.reshape(data_all.shape[0], data_all.shape[1], -1)
        ...
        for i, lbl in enumerate(labels):
            samples_i = data_all[i].reshape(data_all[i].shape[0], -1)
            proj_i = pca.transform(samples_i)
...
def posterior_collapse_check(
    ...
    print(mu_expanded.mean())
    print(mu_expanded.std())
    print(logvar_expanded.shape)
    print(logvar_expanded.mean())
    print(logvar_expanded.std())
    print("----------------------------")
    print(theta_raw.shape)
    print(theta_raw.mean())
    print(theta_raw.std())
    print("##############################\n")
    ...
    print(
        f"  [posterior_collapse_check] N={n_samples} draws from single image posterior"
    )
    print(
        f"    theta_raw shape: {theta_raw.shape}  mu shape: {mu.shape}  logvar shape: {logvar.shape}"
    )

    print("############### Posterior Collapse Check ###############")
    print_collapse_stats("theta_raw", theta_raw.cpu().numpy(), mu.cpu().numpy())
    print_collapse_stats("mu", mu.cpu().numpy(), mu.cpu().numpy())
    print_collapse_stats("logvar", logvar.cpu().numpy(), mu.cpu().numpy())
    print_collapse_stats(
        "theta decoded: ", theta_decoded.cpu().numpy(), mu.cpu().numpy()
    )
    print("########################################################")
    ...
    return {
        ...
        "theta_decode": theta_decoded.cpu().numpy(),
    }
...
def plot_pca_posterior(
    ...
    datasets_to_plot = [
        ("theta_raw", "PCA — θ raw instances"),
        ("theta_decode", "PCA — θ decoded instances"),
    ]

    for ax, (key, title) in zip(axes, datasets_to_plot):
        data = collapse_outputs[key]
        ...
        projected_mean = pca.transform(mean)

        # Gaussian KDE contour fitted to the projected samples
        # Fixed ±3 range, expanded if samples fall outside
        x_min = min(-0.0001, projected[:, 0].min() * 1.1)
        x_max = max(0.0001, projected[:, 0].max() * 1.1)
        y_min = min(-0.0001, projected[:, 1].min() * 1.1)
        y_max = max(0.0001, projected[:, 1].max() * 1.1)
...
def noise_stability_check(
    ...
    z, mu, logvar = model.encode(x)
    z_raw = model.latent_encoder.reparameterize(mu, logvar)

    _, theta = model.decoder.forward_with_weights(z_raw)

    n_cols = len(noise_levels)
    fig, axes = plt.subplots(2, n_cols, figsize=(n_cols * 2.5, 6))

    for col, noise_std in enumerate(noise_levels):
        # Row 1: noise on z_raw → modulate → decode
        noise = torch.randn_like(z_raw) * noise_std
        noised_raw = z_raw + noise
        img_raw = model._decode_latent(noised_raw)

        # Row 2: modulate first → noise on theta → decode
        noise_m = torch.randn_like(theta) * noise_std
        noised_weight = theta + noise_m
        img_mod = model.decoder.forward_from_weights(noised_weight)  # (1, C, H, W)
        ...
        for row, img in enumerate([img_raw, img_mod]):
            ...
            axes[0, 0].set_ylabel("noise on z", fontsize=9)
            axes[1, 0].set_ylabel("noise on θ", fontsize=9)

    for row, lbl in enumerate(["z", "theta"]):
        axes[row, 0].text(
            -0.15,
            0.5,
            lbl,
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze Latent Weight Diffusion Model"
    )
    parser.add_argument(
        "--out_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to model configuration if required",
    )
    parser.add_argument(
        "--weights_path",
        type=str,
    ...
    # 1. Initialize Latent Diffusion Model
    ...
    encoder_outputs = get_encoder_outputs(model, loader, device, args.n_enc_batches)
    ...
    plot_histograms(
        {"theta_raw": encoder_outputs["z"]},
        "theta Raw from encoder",
        args.out_dir,
        "theta_raw",
    )
    plot_histograms(
        {"theta_decoded": encoder_outputs["theta_decoded"]},
        "decoded theta",
        args.out_dir,
        "theta_decoded",
    )
