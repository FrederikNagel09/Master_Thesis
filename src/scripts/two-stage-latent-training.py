...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--skip_vae", action="store_true", default=False, help="Skip VAE training and load pre-trained weights instead")
    p.add_argument("--vae_weights", type=str, default=None, help="Path to _vae_weights.pt (required when --skip_vae is set)")
    ...
    # Fixed-mode epochs
    p.add_argument("--total_epochs", type=int, default=400, help="[fixed mode] Total epochs across both stages")
    p.add_argument("--vae_epochs", type=int, default=100, help="[fixed mode] How many of total_epochs go to VAE training")

    # Convergence-mode VAE stopping
    p.add_argument("--vae_check_every", type=int, default=5, help="[convergence mode] Validate VAE every N epochs")
    p.add_argument("--vae_patience", type=int, default=10, help="[convergence mode] Stop VAE after this many checks without improvement")
    p.add_argument("--vae_delta", type=float, default=1e-4, help="[convergence mode] Minimum ELBO improvement to count as progress")
    p.add_argument("--vae_max_epochs", type=int, default=1000, help="[convergence mode] Hard cap on VAE epochs")

    # Convergence-mode DDPM stopping
    p.add_argument("--ddpm_check_every", type=int, default=5, help="[convergence mode] Validate DDPM every N epochs")
    p.add_argument(
        "--ddpm_patience", type=int, default=20, help="[convergence mode] Window size (in checks) for smoothed plateau detection"
    )
    p.add_argument(
        "--ddpm_delta", type=float, default=1e-4, help="[convergence mode] Minimum avg-loss improvement (first vs second half of window)"
    )
    p.add_argument("--ddpm_max_epochs", type=int, default=5000, help="[convergence mode] Hard cap on DDPM epochs")
...
class TwoStageLDM(nn.Module):
    ...
    def p_sample_loop(self, n_samples: int) -> torch.Tensor:
        ...
        for t_idx in reversed(range(self.T)):
            t_tensor = torch.full((n_samples, 1), t_idx / (self.T - 1), device=self.device, dtype=torch.float32)
...
def save_ddpm_training_graph(
    ...
    if has_fid:
        axes[2].plot(history["fid_epochs"], history["fid_scores"], color="tab:red", linewidth=1.2, marker="o", markersize=4)
...
def train_ddpm(
    ...
    if args.mode == "fixed":
        ...
        detector = None
        print(f"  Fixed mode: {max_epochs} DDPM epochs " f"({args.total_epochs} total - {vae_epochs_done} VAE).")
    else:
...
def compute_final_eval(
    ...
    if is_3d:
        ...
        print(f"  Computing MMD/COV ({generated.shape[0]} generated vs {reference.shape[0]} reference) …")
...
def run_training(args: argparse.Namespace) -> None:
    ...
    if args.skip_vae:
        if not os.path.exists(results_dir):
            raise FileNotFoundError(f"Results directory '{results_dir}' not found. " "Run full training first before using --skip_vae.")
    ...
    if args.skip_vae:
        ...
        print("=" * 60)
        vae = load_pretrained_vae(args.vae_weights, hparams, channels, img_size, device, is_3d=is_3d)
        vae_epochs_done = 0
    else:
        vae = train_vae(
            args,
