...
def build_vae_model(vae_config: dict, channels: int, img_size: int, device: str):
    ...
    from models.latent_diffusion.modules.trans_inr import TransInr
...
def main():
    parser = argparse.ArgumentParser(
        description="Upscaled sample + reconstruction vs interpolation plots."
    )
    parser.add_argument(
        "--config_path",
        type=str,
        required=True,
        help="Path to LDM config.json or VAE _config.json.",
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
        default=None,
        help="Path to VAE checkpoint .pt (required for VAE models).",
    )
    parser.add_argument(
        "--sample_scale",
        type=int,
    ...
    parser.add_argument(
        "--n_samples", type=int, default=10, help="Number of samples in the sample row."
    )
    parser.add_argument(
        "--n_recon",
        type=int,
        default=8,
        help="Number of images in the reconstruction comparison.",
    )
    parser.add_argument(
        "--interp_mode",
        type=str,
        ...
        help="Interpolation method for upscaling originals.",
    )
    ...
    if is_ldm:
        ...
        model_name = (
            hparams.model if hasattr(hparams, "model") else config.get("model", "")
        )
        model_type = "weight_diffusion" if "weight" in model_name.lower() else "ldm"

        print(f"\n{'=' * 60}")
        print(
            f"  Eval Visual [{model_type.upper()}]  |  run={run_name}  |  device={device}"
        )
        print(f"{'=' * 60}\n")
    ...
    print(
        f"  Scale={args.sample_scale}  |  n_samples={args.n_samples}  |  n_recon={args.n_recon}"
    )
