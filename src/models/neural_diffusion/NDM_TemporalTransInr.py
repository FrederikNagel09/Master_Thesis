...
class NDMTemporalTransInr(nn.Module):
    ...
    def negative_elbo(self, x: torch.Tensor) -> torch.Tensor:
        ...
        t_idx = (
            torch.randint(1, self.T + 1, (batch_size,), device=x.device) - 1
        )  # 0-indexed
    ...
    def _l_diff(self, x, theta_t, t_idx, t_norm, theta_prime_t):
        ...
        eps_hat = self.noise_predictor(
            theta_t, t_norm.unsqueeze(1)
        )  # (batch, weight_dim)
        ...
        diff = alpha_s * (Fx_s - Fx_hat_s) + coeff * alpha_t * (
            Fx_hat_t - theta_prime_t
        )
    ...
    def sample_weight(self, n_samples: int = 1) -> torch.Tensor:
        ...
        for t in tqdm(
            range(self.T - 1, -1, -1), desc="NDM Sampling", total=self.T, leave=False
        ):
            t_norm = torch.full(
                (n_samples, 1), t / T_minus_1, device=device
            )  # (n, 1) for noise_predictor
            t_norm_flat = torch.full(
                (n_samples,), t / T_minus_1, device=device
            )  # (n,) for weight_encoder
            ...
            s = t - 1
            s_norm_flat = torch.full(
                (n_samples,), s / T_minus_1, device=device
            )  # (n,) for weight_encoder
            ...
            Fx_2x = self.weight_encoder(
                x_hat_2x, t_norm_2x
            )  # (2*n, weight_dim)  # noqa: N806
            ...
            coeff = (sigma_s_sq - sigma_tilde_sq).clamp(min=0).sqrt() / sigma_t.clamp(
                min=1e-6
            )
            mu = alpha_s * Fx_hat_s + coeff * (z_t - alpha_t * Fx_hat_t)

            z_t = (
                mu + sigma_tilde_sq.clamp(min=0).sqrt() * torch.randn_like(z_t)
