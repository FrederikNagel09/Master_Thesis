...
from models.neural_diffusion.ndm_unet_module import UNetModel
from models.utils.helper_modules import (
    SinusoidalLearnableTimeEmbedding,
...
class UNetTransformation(nn.Module):
    ...
    def __init__(self, t_dim: int = 64, data_dim: int = 784, base_channels: int = 32):
        ...
        self.up3 = nn.ConvTranspose2d(
            chs[4], chs[4], kernel_size=2, stride=2
        )  #  2 ->  4
        self.up2 = nn.ConvTranspose2d(
            chs[3], chs[3], kernel_size=2, stride=2
        )  #  4 ->  8 (trimmed to 7)
        self.up1 = nn.ConvTranspose2d(
            chs[2], chs[2], kernel_size=2, stride=2
        )  #  7 -> 14
        self.up0 = nn.ConvTranspose2d(
            chs[1], chs[1], kernel_size=2, stride=2
        )  # 14 -> 28
...
class NeuralDiffusionModel(nn.Module):
    ...
    def __init__(
        ...
        alpha_cumprod = alpha.cumprod(
            dim=0
        )  # goes to 0 as t increases, since alpha < 1
    ...
    def _l_rec(
        self, x: torch.Tensor, z_t: torch.Tensor, t_idx: torch.Tensor
    ) -> torch.Tensor:
        """
