...
from models.utils.helper_modules import SinusoidalLearnableTimeEmbedding
...
class NoisePredictor(nn.Module):
    ...
    def forward(self, z: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        t_global = self.time_proj(
            t_emb
        )  # (batch, hidden_dim) — for residual conditioning
...
class TransformerNoisePredictor(nn.Module):
    ...
    def forward(self, z: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        if GLOBAL_DEBUG_BOOL and random.random() < probability_threshold:
            ...
            print(
                f"z_in: min={z.min():.4f}, max={z.max():.4f}, mean={z.mean():.4f}, std={z.std():.4f}"
            )
...
class NeuralDiffusionModelINR(nn.Module):
    ...
    def _sigma_tilde_sq(self, s_idx: torch.Tensor, t_idx: torch.Tensor) -> torch.Tensor:
        ...
        base = (
            (sigma_t_sq - alpha_t_sq / alpha_s_sq * sigma_s_sq)
            * sigma_s_sq
            / sigma_t_sq
        )
    ...
    def _inr_decode(
        self, flat_weights: torch.Tensor, coords: torch.Tensor | None = None
    ) -> torch.Tensor:
        """
        ...
        if (
            GLOBAL_DEBUG_BOOL and random.random() < probability_threshold
        ):  # print debug info for ~0.1% of calls
            print("==================== DEBUG: inr_decode.py ====================")
            print(f"Decoded pixels shape: {pixels.shape}")
            print(
                f"Pixel value range: {pixels.min().item():.4f} to {pixels.max().item():.4f}"
            )
            print("================================================================")
    ...
    def negative_elbo(self, x: torch.Tensor):
        ...
        if (
            GLOBAL_DEBUG_BOOL and random.random() < probability_threshold
        ):  # print debug info for ~0.1% of batches
            print("==================== DEBUG: NDM_INR.py ====================")
            print(f"t_norm (normalized time): {t_norm.min():.4f} to {t_norm.max():.4f}")
            print(
                f"t_idx (time step indices): {t_idx.min().item()} to {t_idx.max().item()}"
            )
            print("================================================================")
    ...
    @torch.no_grad()
    def decode_weights(
        self, weights: torch.Tensor, coords: torch.Tensor | None = None
    ) -> torch.Tensor:
        return self._inr_decode(weights, coords)

    @torch.no_grad()
    def sample(
        self, n_samples: int = 1, coords: torch.Tensor | None = None
    ) -> torch.Tensor:
...
class NDMTemporalINR(NeuralDiffusionModelINR):
    ...
    def _l_diff(self, x, z_t, t_idx, t_norm, Fx_t):  # noqa: N803
        ...
        coeff = (self.sigma_sq[s_idx].unsqueeze(1) - sigma_tilde_sq).clamp(
            min=0
        ).sqrt() / self.sigma[t_idx].unsqueeze(1).clamp(min=1e-6)

        diff = alpha_s * (Fx_s - Fx_hat_s) + coeff * alpha_t * (Fx_hat_t - Fx_t)
        l_diff = (diff**2).sum(dim=-1) / (
            2.0 * sigma_tilde_sq.squeeze(1).clamp(min=1e-8)
        )
        return l_diff
    ...
    def sample_weight(self, n_samples: int = 1) -> torch.Tensor:
        ...
        for t in tqdm(
            range(self.T - 1, -1, -1), desc="NDM (Temporal) Sampling", total=self.T
        ):
            ...
            coeff = (sigma_s_sq - sigma_tilde_sq).clamp(
                min=0
            ).sqrt() / sigma_t_val.clamp(min=1e-6)

            mu = alpha_s * Fx_hat_s + coeff * (theta_t - alpha_t_val * Fx_hat_t)
            noise = (
                torch.randn_like(theta_t)
                if sigma_tilde_sq.item() > 0
...
class NDMStaticINR(NeuralDiffusionModelINR):
    ...
    def _sample_zt(self, x, t_idx, t_norm):  # noqa: ARG002
        ...
        if GLOBAL_DEBUG_BOOL and ran:
            ...
            print(
                f"x (input images): min {x.min().item():.4f}, max {x.max().item():.4f}"
            )
            print("================================================================")
        ...
        if GLOBAL_DEBUG_BOOL and ran:
            ...
            print(
                f"W(x) (encoded weights): min {theta.min().item():.4f}, max {theta.max().item():.4f}"
            )
        ...
        if GLOBAL_DEBUG_BOOL and ran:
            ...
            print(
                f"z_t (noisy images): min {theta_t.min().item():.4f}, max {theta_t.max().item():.4f}"
            )
            print("================================================================")
    ...
    def _l_diff(self, x, z_t, t_idx, t_norm, Wx):  # noqa: ARG002, N803
        ...
        if GLOBAL_DEBUG_BOOL and random.random() < probability_threshold:
            ...
            print(
                f"eps_hat (predicted noise): min {eps_hat.min().item():.4f}, max {eps_hat.max().item():.4f}"
            )
        ...
        if GLOBAL_DEBUG_BOOL and random.random() < probability_threshold:
            ...
            print(
                f"x_hat_pixels (reconstructed images): min {x_hat_pixels.min().item():.4f}, max {x_hat_pixels.max().item():.4f}"
            )
        ...
        if GLOBAL_DEBUG_BOOL and random.random() < probability_threshold:
            ...
            print(
                f"W(x_hat) (re-encoded weights): min {Wx_hat.min().item():.4f}, max {Wx_hat.max().item():.4f}"
            )
        ...
        sigma_tilde_sq = self._sigma_tilde_sq(s_idx, t_idx).unsqueeze(1)
        B = (  # noqa: N806
            (self.sigma_sq[s_idx].unsqueeze(1) - sigma_tilde_sq).clamp(min=0).sqrt()
            / self.sigma[t_idx].unsqueeze(1).clamp(min=1e-6)
        )
    ...
    def _l_prior(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if GLOBAL_DEBUG_BOOL and random.random() < probability_threshold:
            ...
            print(
                f"W(x) at T (encoded weights): min {Wx_T.min().item():.4f}, max {Wx_T.max().item():.4f}"
            )
    ...
    def sample_weight(self, n_samples: int = 1) -> torch.Tensor:
        ...
        for t in tqdm(
            range(self.T - 1, -1, -1), desc="NDM (Static) Sampling", total=self.T
        ):
            t_idx = torch.full((n_samples,), t, dtype=torch.long, device=device)
            t_norm = torch.full((n_samples, 1), t / max(self.T - 1, 1), device=device)
            ...
            B = (sigma_s_sq - sigma_tilde_sq).clamp(min=0).sqrt() / sigma_t_val.clamp(
                min=1e-6
            )  # noqa: N806
