...
class LatentScaler(nn.Module):
    ...
    def forward(
        self, z: torch.Tensor, reverse: bool = False, training: bool = True
    ) -> torch.Tensor:
        """
...
class LatentDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def sample(
        self, n_samples: int = 1, collect_snapshots: bool = False, debug: bool = True
    ) -> torch.Tensor:
        """
        ...
        if collect_snapshots:
            z, snapshots = self._sample_latent(
            ...
            z = self._sample_latent(
                n_samples, collect_snapshots=collect_snapshots, debug=debug
            )  # (B, latent_dim, H', W')
    ...
    def _negative_elbo(self, x: torch.Tensor) -> tuple[torch.Tensor, ...]:
        ...
        if mask_tdiff.any():
            l_diff[mask_tdiff], unscaled_loss = self._l_diff(
                z_t[mask_tdiff],
                t_norm[mask_tdiff],
                epsilon[mask_tdiff],
                t_idx[mask_tdiff],
        ...
        if mask_t0.any():
            l_latent_rec[mask_t0] = self._l_latent_rec(
                z_t[mask_t0], t_norm[mask_t0], z[mask_t0]
            )
        ...
        scale = self.i / 10000.0 if self.i < 10000 else 1.0
    ...
    def _l_diff(
        ...
        scaling = beta_t / (2 * alpha_t * (1.0 - alpha_bar_t))  # noqa: F841
    ...
    def _l_latent_rec(
        ...
        return F.mse_loss(mu_theta, z_clean_t0, reduction="none").mean(
            dim=[1, 2, 3]
        ) / (2.0 * beta_0)
    ...
    @torch.no_grad()
    def _sample_latent(
        self, n_samples: int, collect_snapshots: bool = False, debug: bool = True
    ) -> torch.Tensor:
        """
    ...
    def print_encoded_stats(
        self, logvar: torch.Tensor, mu: torch.Tensor, z_raw: torch.Tensor
    ) -> None:
        """
        ...
        if GLOBAL_DEBUG_BOOL and random.random() < probability_threshold:
            ...
            print(
                f"[Encoder] mu:     mean={mu.mean():.3f}, std={mu.std():.3f}, min={mu.min():.3f}, max={mu.max():.3f}"
            )
            print(
        ...
        if GLOBAL_DEBUG_BOOL and random.random() < probability_threshold:
            ...
            if self.i % 10 == 0:
                ...
                for t in t_idx_debug:
                    ...
                    print(
                        f"t={idx:3d}/{self.T}: mean={z_t.mean():.4f}, std={z_t.std():.4f}"
                    )
    ...
    def print_debug_info(
        self, epsilon, eps_hat, mse, l_diff_loss, debug: bool = True
    ) -> None:
    ...
    def print_sampling_stats(self, t, eps_hat, z, debug: bool = True) -> None:
        ...
        if debug and GLOBAL_DEBUG_BOOL and (t % 100 == 0 or t == 0):
            ...
            print(
                f"z stats: mean={z.mean():.4f}, std={z.std():.4f}",
    ...
    def print_masking_debug(
        ...
        print(
            f"t=0  samples:      {n_t0}  ({100*n_t0/B:.1f}%,  expected ~{100/self.T:.1f}%)"
        )
        print(
            f"t>0  samples:      {n_tdiff} ({100*n_tdiff/B:.1f}%, expected ~{100*(self.T-1)/self.T:.1f}%)"
        )
