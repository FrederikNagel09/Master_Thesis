...
class LatentEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.cnn = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(
                hidden_dim * 2, bottleneck_dim, kernel_size=3, stride=2, padding=1
            ),
            nn.GELU(),
            # Decoder: upsample → upsample → project to latent_dim
            nn.ConvTranspose2d(
                bottleneck_dim, hidden_dim * 2, kernel_size=4, stride=2, padding=1
            ),
            nn.GELU(),
            nn.ConvTranspose2d(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if z.shape[-2:] != self.latent_size:
            z = F.interpolate(
                z, size=self.latent_size, mode="bilinear", align_corners=False
            )
...
class ResNetLatentEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.upsample_mu = nn.ConvTranspose2d(
            in_channels=ch4, out_channels=latent_dim, kernel_size=4, stride=2, padding=1
        )
        self.upsample_logvar = nn.ConvTranspose2d(
            in_channels=ch4, out_channels=latent_dim, kernel_size=4, stride=2, padding=1
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if mu.shape[-2:] != self.latent_size:
            mu = nn.functional.interpolate(
                mu, size=self.latent_size, mode="bilinear", align_corners=False
            )
            logvar = nn.functional.interpolate(
                logvar, size=self.latent_size, mode="bilinear", align_corners=False
            )
