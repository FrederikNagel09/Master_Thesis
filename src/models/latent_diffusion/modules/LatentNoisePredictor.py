...
class LatentUNetNoisePredictor(nn.Module):
    ...
    def __init__(
        ...
        assert (
            self.spatial_dim * self.spatial_dim == n_patches
        ), "n_patches must be a perfect square"
...
def get_2d_sincos_pos_embed(embed_dim, grid_size):
    ...
    )  # Now perfectly outputs (1, H*W, embed_dim)
...
class LatentTransformerNoisePredictor(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                DiTBlock(d_model, n_heads, d_ff, dropout, t_dim=d_model)
