...
from src.models.tokenizers.hyperdiff_tokenizer import (
    HyperDiffDetokenizer,
    HyperDiffTokenizer,
)
from src.models.tokenizers.param_tokenizer import (
    ParamDetokenizer,
    ParamTokenizer,
)
...
class ParamDiTBlock(nn.Module):
    ...
    def forward(self, x, cond):
        shift_attn, scale_attn, gate_attn, shift_mlp, scale_mlp, gate_mlp = self.ada(
            cond
        ).chunk(6, dim=-1)
...
class ParamDiT(nn.Module):
    ...
    def __init__(
        ...
        if tokenizer in {"column", "param", "columns"}:
            ...
            raise ValueError(
                f"Unknown ParamDiT tokenizer '{tokenizer}'. "
                "Expected one of: column, hyperdiff."
            )
    ...
    def forward(self, params, timesteps):
        ...
        cond = self.time_embed(
            timestep_embedding(timesteps, self.time_dim).to(params.dtype)
        )
