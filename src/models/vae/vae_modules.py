...
class MoGPrior(nn.Module):
    def __init__(self, latent_dim, gauss_components=10):
        ...
        self.mixture_weights = nn.Parameter(
            torch.ones(gauss_components) / gauss_components, requires_grad=True
        )
        self.means = nn.Parameter(
            torch.randn(gauss_components, latent_dim), requires_grad=True
        )
        self.stds = nn.Parameter(
            torch.ones(gauss_components, latent_dim), requires_grad=True
        )
...
class VAMPPrior(nn.Module):
    def __init__(self, encoders, input_shape, num_components):
        ...
        self.pseudo_inputs = nn.Parameter(
            torch.randn(num_components, *input_shape) * 0.1
        )
...
class BernoulliFullDecoder(nn.Module):
    ...
    def __init__(
        self, latent_dim: int, output_shape: tuple[int, int], hidden_dims: list[int]
    ):
        super().__init__()
        layers = []
        in_dim = latent_dim
