...
class ParamNormalizer(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, reverse: bool = False, training: bool = True
    ) -> torch.Tensor:
        """
        Args:
...
class WeightDiffusion(nn.Module):
    ...
    def __init__(
        ...
        if self.normalize:
            self.scaler = ParamNormalizer(
                WeightEncoder.modulation_dim, momentum=0.01, eps=1e-6
            )
        self.i = 0
    ...
    def sample(
        ...
        if collect_snapshots:
            theta, snapshots = self.sample_weight(
    ...
    def negative_elbo(self, x: torch.Tensor) -> torch.Tensor:
        ...
        theta_prime = theta_prime = (
            self.scaler(theta_prime_raw, reverse=False)
            if self.normalize
        ...
        if False:
            ...
            if mask_tdiff.any():
                l_diff[mask_tdiff] = self._l_diff(
                    ...
                    t_norm[mask_tdiff],
                    epsilon[mask_tdiff],
                    theta_prime[mask_tdiff],
                    t_idx[mask_tdiff],
                )

            # 3. Compute Weight Reconstruction Loss only where t == 0
            if mask_t0.any():
                l_weight_rec[mask_t0] = self._l_weight_rec(
                    theta_t[mask_t0], t_norm[mask_t0], theta_prime[mask_t0]
                )
        else:
    ...
    def compute_full_elbo(self, val_loader: torch.utils.data.DataLoader) -> float:
        ...
        for x, _ in val_loader:
            ...
            theta_prime = (
                theta_prime.detach() if self.stop_gradient_flow else theta_prime
            )
            ...
            for t in range(self.T):
                ...
                l_diff_sum += self._l_diff(
                    theta_t, t_norm, epsilon, theta_prime, t_idx, debug=False
                )  # (B,)
    ...
    def _l_entropy(self, logvar: torch.Tensor) -> torch.Tensor:
        ...
        entropy_per_dim = 0.5 * (
            1.0
            + torch.log(torch.as_tensor(2.0 * math.pi, device=logvar.device))
            + logvar
        )
        return entropy_per_dim.mean(
            dim=-1
        )  # (B,) — sum over latent dims, return positive entropy
    ...
    def sample_weight(
        ...
        for t in tqdm(range(self.T - 1, -1, -1), desc="Sampling", total=self.T):
            t_norm = torch.full(
                (n_samples,), t / (self.T - 1), device=device
            ).unsqueeze(-1)
            ...
            eps_hat = (
                sqrt_1mab * curr_theta + sqrt_ab * v_hat
            )  # (n_samples, weight_dim)
            ...
            curr_theta = (
                mean + torch.sqrt(beta_t) * torch.randn_like(curr_theta)
                if t > 0
                else mean
            )
    ...
    def print_encoder_stats(self, mean, logvar, theta_prime_raw):
        if GLOBAL_DEBUG_BOOL and random.random() < probability_threshold:
            ...
            print(
                f"==================== Probablistic Components {self.i}: ===================="
            )
            print(
                f"[Encoder] mu:     mean={mean.mean():.3f}, std={mean.std():.3f}, min={mean.min():.3f}, max={mean.max():.3f}"
            )
            print(
                f"[Encoder] logvar: mean={logvar.mean():.3f}, std={logvar.std():.3f}, min={logvar.min():.3f}, max={logvar.max():.3f}"
            )
            print(
                f"[Encoder] std:    mean={std.mean():.3f}, std={std.std():.3f}, min={std.min():.3f}, max={std.max():.3f}"
            )
            print(
        ...
        if GLOBAL_DEBUG_BOOL and random.random() < probability_threshold:
            ...
            if self.i % 10 == 0:
                ...
                t_idx_debug = torch.tensor(
                    t_steps, dtype=torch.long, device=theta_prime_raw.device
                )
                ...
                for t in t_idx_debug:
                    ...
                    print(
                        f"DEBUG SAMPLE t={idx:3d}: mean={theta_t.mean():.4f}, std={theta_t.std():.4f}"
                    )

                print("###############################################\n")
                print(
                    f"DEBUG SCALED THETA: mean={theta_prime_raw.mean():.4e}, std={theta_prime_raw.std():.4e}"
                )
                print(
                    f"Debug range of scaled theta_prime: min={theta_prime_raw.min().item():.4f}, max={theta_prime_raw.max().item():.4f}"
                )
        self.i += 1
    ...
    def print_sampling_stats(self, v_hat, x0_hat, curr_theta, t, debug=True):
        if (t % 100 == 0 and GLOBAL_DEBUG_BOOL and debug) or (
            t == 0 and GLOBAL_DEBUG_BOOL and debug
        ):
            print("################## Sampling: ##############################")
