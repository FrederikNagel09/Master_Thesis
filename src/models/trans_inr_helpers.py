...
class VolumeTokenizer(nn.Module):
    ...
    def __init__(self, in_channels, vol_size, patch_size, dim, n_head, head_dim, padding=0):
        super().__init__()
        ...
        n_patches = ((D + 2*pad_d) // pd) * ((H + 2*pad_h) // ph) * ((W + 2*pad_w) // pw)
        self.posemb = nn.Parameter(torch.randn(1, n_patches, dim))
    ...
    def forward(self, x):
        ...
        x = F.unfold(x, kernel_size=(pd * ph, pw), stride=(pd * ph, pw), padding=(self.padding[0]*self.padding[1], self.padding[2]))
